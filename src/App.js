...
function App() {
  ...
  const calculate = async () => {
    ...
    const reserve0 = await token0Instance.methods.balanceOf(pairAddress).call();
    const reserve1 = await token1Instance.methods.balanceOf(pairAddress).call();

    const lpTotalSupply = await uniPair.methods.totalSupply().call();

    var userToken0Share = new BN(reserve0)
      .mul(new BN(lpAmount).mul(new BN("10").pow(new BN("18"))))
      .div(new BN(lpTotalSupply));
    var userToken1Share = new BN(reserve1)
      .mul(new BN(lpAmount).mul(new BN("10").pow(new BN("18"))))
      .div(new BN(lpTotalSupply));

    userToken0Share = await toDecimal(token0Instance, userToken0Share);
    userToken1Share = await toDecimal(token1Instance, userToken1Share);

    setToken0Share(userToken0Share);
    setToken1Share(userToken1Share);
  ...
  return (
    <Grid container direction="column">
      ...
      <Grid item>Token A Val: {token0Share} {token0Name}</Grid>
      <Grid item>Token B Val: {token1Share} {token1Name}</Grid>
    </Grid>
  );
}
